...
export default function SellMaterial({ onListMaterial }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const phoneRegex = /^[0-9]{10}$/; // Adjust as per format requirement
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-xl shadow-sm p-8">
          ...
          <div className="flex space-x-4 mb-8">
            ...
            <div className="relative flex items-center bg-gray-200 rounded-full p-1 w-[250px]">

              <div
                className={`absolute top-1 left-1 h-10 w-[120px] rounded-full bg-green-600 transition-transform duration-300
      ${section === 'sustainable' ? 'translate-x-[124px]' : 'translate-x-0'}`}
              ></div>


              <button
                onClick={() => setSection('raw')}
                className={`z-10 w-[120px] h-10 text-sm font-semibold rounded-full transition-all duration-300
      ${section === 'raw' ? 'text-white' : 'text-gray-800'}`}
              >
                Raw Materials
              </button>

              {/* Sustainable Products Button (multi-line) */}
              <button
                onClick={() => setSection('sustainable')}
                className={`z-10 w-[120px] h-10 text-sm font-semibold rounded-full transition-all duration-300 text-center
      ${section === 'sustainable' ? 'text-white' : 'text-gray-800'}`}
              >
                Sustainable<br />Products
              </button>
            </div>
