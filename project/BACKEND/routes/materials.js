...
router.post("/", async (req, res) => {  
    try {
        const { name, category, price, quantity, unit, location, description, section, image, phone, email, seller } = req.body;

        if (!name || !category || !price || !quantity || !unit || !location || !description || !phone || !email) {
            return res.status(400).json({ message: "All fields are required!" });
        }
